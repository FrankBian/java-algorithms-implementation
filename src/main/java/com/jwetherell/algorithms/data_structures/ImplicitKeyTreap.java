...
public class ImplicitKeyTreap<T> implements IList<T> {
    ...
    public static <T> Pair<T> split(Node<T> node, int index) {
        ...
        final int leftSize = (node.left != null ?
                ...
                0);
        if (index <= leftSize) {
            final Pair<T> sub = split(node.left, index);
            node.left = sub.right;
            if (node.left != null)
        ...
        // else
        final Pair<T> sub = split(node.right, (index - leftSize - 1));
        node.right = sub.left;
        if (node.right != null)
    ...
    private static <T> Node<T> insert(Node<T> root, int index, T value) {
        ...
        return merge(merge(p.left, new Node<T>(value)), p.right);
    }
    ...
    private static <T> Node<T> getNodeByIndex(Node<T> node, int parentIndex, int index) {
        ...
        final Node<T> p = node.parent;
        final Node<T> l = node.left;
        final Node<T> r = node.right;
    ...
    private Node<T> getNodeByIndex(int index) {
        ...
        final Node<T> l = root.left;
        final Node<T> r = root.right;
        final int leftSize = ((l != null) ? l.size : 0);
